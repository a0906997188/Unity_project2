...
public class PlayerBunnyController : player
{
    ...
    void Init_Charactor()
    {
        lv = (CharactorLevel)GamePlayerMemory.BunnyLevel;
        ToolScript.ChangeValueRelyOnLevel(out Attack, lv, 10, 12, 14, 16, 18);
        ToolScript.ChangeValueRelyOnLevel(out maxHealth, lv, 50, 60, 70, 80, 100);
        ToolScript.ChangeValueRelyOnLevel(out creatMoney, lv, 10, 10, 12, 12, 14);
    ...
    {
        RaycastHit2D[] hits = Physics2D.RaycastAll(transform.position, Vector2.right, 0.7f);
